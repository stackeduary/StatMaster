    ...
    <template v-if="!selectedTeam">
      ...
      <div class="flex-1 px-4 py-4 max-w-lg mx-auto w-full">
        <div v-if="teams.length === 0" class="text-center py-16">
          ...
          <p class="text-muted-foreground mb-4">No teams yet</p>
          <button 
        ...
        <div v-else class="space-y-3">
          <button
            ...
            class="w-full bg-card rounded-xl p-4 shadow-sm text-left hover:shadow-md transition-shadow flex items-center justify-between"
          >
            <div>
              <p class="font-semibold text-foreground">{{ team.name }}</p>
              <p class="text-sm text-muted-foreground">{{ team.players.length }} players</p>
            </div>
      ...
      <div class="flex-1 px-4 py-4 max-w-lg mx-auto w-full">
        <div class="bg-card rounded-xl shadow-sm overflow-hidden">
          <div class="px-4 py-3 border-b border-border flex items-center justify-between">
            <h3 class="font-semibold text-foreground">Roster</h3>
            <button 
          ...
          <div v-if="selectedTeam.players.length === 0" class="p-8 text-center text-muted-foreground">
          ...
          <div v-else class="divide-y divide-border">
            ...
            >
              <div class="flex items-center gap-3">
                <span class="w-10 h-10 bg-primary/10 text-primary rounded-full flex items-center justify-center font-bold">
                  {{ player.number || '?' }}
                </span>
                <div>
                  <p class="font-medium text-foreground">{{ player.firstName }} {{ player.lastName }}</p>
                  <p class="text-sm text-muted-foreground">{{ player.position || 'No position' }}</p>
                </div>
              </div>
              <button 
                @click="removePlayer(index)"
                class="p-2 hover:bg-destructive/10 rounded-full text-destructive"
              >
    ...
    >
      <div class="bg-card w-full max-w-lg rounded-t-2xl p-4 pb-8">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-lg font-semibold text-foreground">Add Team</h3>
          <button @click="showAddTeam = false" class="p-2 hover:bg-muted rounded-full">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-foreground mb-1">Team Name</label>
            <input 
              v-model="newTeamName"
              type="text" 
              placeholder="Enter team name"
              class="w-full px-3 py-2 bg-input border border-border rounded-lg focus:ring-2 focus:ring-ring text-foreground"
            />
          </div>
          ...
          <button 
            ...
            class="w-full bg-primary text-primary-foreground py-3 rounded-lg font-medium hover:opacity-90 disabled:opacity-50"
          >
            Create Team
          </button>
        </div>
